...
import { Lazy } from "@framework/core/cache";
import { migrations } from "./migrations";
import { Serializer } from "./serializer";
import { GameLoop } from "@framework/core/game-loop";
import { simulateTime } from "@framework/core/offline-simulation";
import { EventHub, GAME_EVENT, GameUI } from "@framework/core/event-hub";
  ...
  actions: {
    ...
    updatePlayerData() {
      ...
      for (const layer of Layer.all) {
        if (!layer.milestones) continue;
        for (const milestone of layer.milestones.all) {
          milestone.syncReachState();
        }
      }
      const player = usePlayerStore().player;
