...
export default {
  ...
  12: {
    ...
    cost: DC.D2,
    isUnlocked: () => Layer.alpha.upgrades[11].isBought
  },
  13: {
    id: 13,
    title: "Self Powered",
...
// if (Layer.alpha.upgrades[x].isBought) effect = effect.times(Layer.alpha.upgrades[x].effectValue);
// We uses this:
// effect = effect.timesEffectOf(Layer.alpha.upgrades[x].isBought);
// When we use more effects, we can write 
// effect = effect.timesEffectsOf(
//    Layer.alpha.upgrades[x].isBought,
//    Layer.alpha.upgrades[y].isBought,
// );
// Sometimes we use Layer.alpha.upgrades[x].effectOrDefault(DC.D0)
